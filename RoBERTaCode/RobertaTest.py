...
if os.path.exists('/home/sslater/data/testwindowsimproved.csv'):
    data_path = '/home/sslater/data/testwindowsimproved.csv'
else:
    data_path = '/Users/stevenslater/Desktop/FinalProject/Data/testwindowsimproved.csv'
...
# load the model
model.load_state_dict(torch.load('newWindowsRoberta1epoch.pth'))
...
def predict(df):
    ...
    with torch.no_grad():
        for index, row in tqdm(df.iterrows(), total=len(df), desc="Evaluating"):
            ...
            # tokenize the conversation chunk and convert to tensor
...
predictions, actuals = predict(test_df)
